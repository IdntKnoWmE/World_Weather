...
export default function MapLegend() {
    ...
    const maxValue = data["stopsMaxValue"];

    const gradientStops: string = data.stops.map(stop => {
        return `${stop.color} ${(stop.value / maxValue) * 100}%`
    }).join(", ");

    return (
        <div className="absolute top-4 right-2 z-50 w-80
                        rounded-xl shadow-xl p-2 bg-background/60
                        border border-accent/50
                        flex flex-col gap-3"
        >
            <h3 className="text-sm font-semibold text-foreground">{data.title}</h3>
            <div className="w-full h-3 rounded-xl border border-accent/80"
                style={{
                    background: `linear-gradient(to right, ${gradientStops}`
                }}
            />
            <div className="flex justify-between text-xs text-foreground">
                <span>{data.stopsMinValue} {data.unit}</span>
                <span>{data.stopsMaxValue} {data.unit}</span>
            </div>
...
interface MapLegendRecord {
    title: string;
    unit: string;
    stopsMinValue: number;
    stopsMaxValue: number;
    stops: ColorStop[];
}

const mapTypeData: Record<string, MapLegendRecord> = {
    precipitation_new: {
        title: "Rain (mm)",
        unit: "mm",
        stopsMinValue: 0,
